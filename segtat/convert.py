...
def convert_geotiff_into_pt(features_path: str, label_path: str, save_path: str,
    ...
    for i, file in enumerate(label_files):
        ...
        if additional_paths is not None:
            ...
            jrc_occurrence_path = os.path.join(
                additional_paths['jrc_occurrence'], file)
            jrc_recurrence_path = os.path.join(
                additional_paths['jrc_recurrence'], file)
            jrc_seasonality_path = os.path.join(
                additional_paths['jrc_seasonality'], file)
            jrc_transitions_path = os.path.join(
                additional_paths['jrc_transitions'], file)
            nasadem_path = os.path.join(additional_paths['nasadem'], ''.join(
                (file[:-4], '_nasadem.tif.tif')))
            ...
            if i == 0:
                ...
                scaler_occurrence.fit(
                    np.ravel(occurrence_matrix).reshape((-1, 1)))

                # For recurrence
                scaler_recurrence = StandardScaler()
                scaler_recurrence.fit(
                    np.ravel(recurrence_matrix).reshape((-1, 1)))

                # For seasonality
                scaler_seasonality = StandardScaler()
                scaler_seasonality.fit(
                    np.ravel(seasonality_matrix).reshape((-1, 1)))
                ...
                # Save scalers into folder
                pickle.dump(scaler_change,
                            open(os.path.join(save_path, 'scaler_change.pkl'),
                                 'wb'))
                pickle.dump(scaler_extent,
                            open(os.path.join(save_path, 'scaler_extent.pkl'),
                                 'wb'))
                pickle.dump(scaler_occurrence, open(
                    os.path.join(save_path, 'scaler_occurrence.pkl'), 'wb'))
                pickle.dump(scaler_recurrence, open(
                    os.path.join(save_path, 'scaler_recurrence.pkl'), 'wb'))
                pickle.dump(scaler_seasonality, open(
                    os.path.join(save_path, 'scaler_seasonality.pkl'), 'wb'))
                pickle.dump(scaler_transitions, open(
                    os.path.join(save_path, 'scaler_transitions.pkl'), 'wb'))
                pickle.dump(scaler_nasadem,
                            open(os.path.join(save_path, 'scaler_nasadem.pkl'),
                                 'wb'))
                pickle.dump(scaler_swi,
                            open(os.path.join(save_path, 'scaler_swi.pkl'),
                                 'wb'))

            change_transformed = _scale_matrix(scaler_change, change_matrix)
            extent_transformed = _scale_matrix(scaler_extent, extent_matrix)
            occurrence_transformed = _scale_matrix(scaler_occurrence,
                                                   occurrence_matrix)
            recurrence_transformed = _scale_matrix(scaler_recurrence,
                                                   recurrence_matrix)
            seasonality_transformed = _scale_matrix(scaler_seasonality,
                                                    seasonality_matrix)
            transitions_transformed = _scale_matrix(scaler_transitions,
                                                    transitions_matrix)
            ...
            stacked_matrix = np.array(
                [change_transformed, extent_transformed, occurrence_transformed,
                 recurrence_transformed, seasonality_transformed,
                 transitions_transformed, nasadem_transformed, swi_transformed])
        else:
